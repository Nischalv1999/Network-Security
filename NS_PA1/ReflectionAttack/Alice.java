...
// Generating K-alice key and initialising Cipher
  static {
    try {
      ...
      encryptCipherKA = Cipher.getInstance("TripleDES/ECB/PKCS5Padding");

      encryptCipherKA.init(Cipher.ENCRYPT_MODE, KAlice_KDC);
...
// a.start() in Multithreading class runs this function
  ...
  Byte[] encryptTicketObject = new Byte[ticket.length];
    ...
    msgToTrudy.add(Kab);
    // System.out.println("Msg in alice nonce: "+Arrays.toString((Byte[]) msgToTrudy.get(0)));
    // System.out.println("Msg in alice nonce: "+msgToTrudy.get(0));
    // System.out.println("Msg in alice ticket: "+msgToTrudy.get(1));

      // create an object output stream from the output stream so we can send an object to Trudy.
...
// System.out.println("N2 in alice: "+N2);
   try {
    CipherKANonce = Cipher.getInstance("TripleDES/ECB/PKCS5Padding");
    CipherKANonce.init(Cipher.ENCRYPT_MODE, Kab);
...
System.out.println("Ticket after decoding: "+Arrays.toString(this.ticket));
  ...
  private List<Object> msgFromKDC(ArrayList<Object> out_msg2, Socket clientSocket) {
    try {
      ...
      //adding msg-1 to the the object out_msg, to send it to KDC
      out_msg.add(N1);
